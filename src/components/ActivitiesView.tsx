...
export default function ActivitiesView() {
  ...
  const [events, setEvents] = useState<any[]>(UPCOMING_EVENTS); // Fallback to static if loading
  ...
  useEffect(() => {
    async function loadEvents() {
      ...
      try {
        ...
        if (!error && data) {
          setEvents(data.filter(e => e.stage === "upcoming"));
          setArchives(data.filter(e => e.stage === "completed"));
        }
      } catch (e) {
        console.error(e);
      }
  ...
  return (
    ...
    >
      ...
      <section className="space-y-6">
        ...
        >
          {events.length === 0 ? (
            <div className="text-zinc-500 font-mono text-xs w-full text-center py-10 border border-zinc-800 rounded bg-zinc-900/30">NO UPCOMING EVENTS LOGGED IN DATABASE</div>
            ...
            >
              ...
              <div className="h-48 w-full bg-[#0c0c0e] relative overflow-hidden flex items-center justify-center border-b border-zinc-800">
                ...
                <div className="absolute top-3 right-3 bg-[#101010]/90 backdrop-blur-sm border border-zinc-800 px-3 py-1 rounded flex items-center gap-2 z-10">
                  ...
                  <span className="font-mono text-[10px] text-zinc-300 uppercase tracking-[0.1em] font-semibold">{event.status || "Upcoming"}</span>
                </div>
              </div>
              ...
              <div className="p-6 flex-grow flex flex-col justify-between z-10 relative">
                <div className="space-y-4">
                  ...
                  <p className="font-sans text-sm text-zinc-400 leading-relaxed line-clamp-3">{event.description || event.desc}</p>
                </div>
                <div className="mt-6 flex flex-col gap-2">
                  ...
                  <a 
                    href={event.registration_link || event.registrationLink || "https://eventhubcc.vit.ac.in/EventHub/"}
                    target="_blank"
      ...
      <section className="space-y-12 border-t border-dashed border-zinc-800 pt-16">
        ...
        {(() => {
          const years = Array.from(new Set(archives.map(a => a.year))).filter(Boolean).sort().reverse();
          if (years.length === 0) {
            return <div className="text-zinc-500 font-mono text-sm">No archives found.</div>;
          }
          return years.map((yearStr) => (
            <div key={yearStr} className="space-y-6">
              <div className="flex items-center gap-4">
                <h3 className="font-mono text-xl text-[#e8b828] font-bold">{yearStr}</h3>
                <div className="h-px bg-zinc-800 flex-grow border-t border-dashed border-zinc-700"></div>
              </div>
              
              <div className="flex flex-col gap-4 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
                {archives.filter(item => item.year === yearStr).map((item, idx) => {
                    ...
                    >
                      <div className="w-full sm:w-48 h-40 sm:h-auto bg-[#0c0c0e] relative shrink-0">
                        {item.image_url || item.image ? (
                          <img src={item.image_url || item.image} alt={item.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                        ) : (
                      ...
                      <div className="p-6 flex flex-col justify-center flex-grow space-y-2">
                        ...
                        <p className="font-sans text-sm text-zinc-400 leading-relaxed max-w-2xl">{item.description || item.desc}</p>
                      </div>
                      {item.link && (
